...
import web.teachers.day09.base.Base_Test_Case;
 ...
 *  Map:A树  苹果A
 */
public class Register_Success_Test_Case2 extends Base_Test_Case {
	...
	public void register_success_test_case(RegisterInfo registerInfo) throws InterruptedException {
		...
		//输入手机号
		type(REGISTER_PAGE_NAME, "手机号码输入框", registerInfo.getMobilePhone());
		//输入密码
		type("注册页面", "密码输入框", registerInfo.getPwd());
		//输入重复密码
		type("注册页面", "重复密码输入框", registerInfo.getConfirmPwd());
		//输入验证码
		type("注册页面", "验证码输入框", registerInfo.getVerifycode());
		//单击注册按钮
		click("注册页面", "注册按钮");
		//断言:页面上面出现登录元素
		WebElement loginBtn = getElement(LOGIN_PAGE_NAME,"登录按钮");
